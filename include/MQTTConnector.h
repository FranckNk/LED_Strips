...
bool getState(String val){
  if (!val.compareTo("true"))
    return true;
  else 
    return false;
}
void messageReceived(String &topic, String &payload){
  ...
  input = payload.substring(payload.indexOf(":") + 2, payload.indexOf(",") - 1);
  
  Serial.println("DataRecu : " + DataRecu);
  Serial.println("input : " + input);
  
  if (input == "ButonState"){
   DataRecu = DataRecu.substring(1, 4);
    Serial.println(DataRecu);
    //SB1 = DataRecu.toInt(); // Variable qui définira la couleur de la LED
    Serial.println(SB1);
  }
  else if (input == "Speed"){
    Speed = DataRecu.toInt();
  }
